...
public class OrderService {
    ...
    public OrderPaymentResponse createOrder(HttpServletRequest servletRequestRequest, CreateOrderRequest request) {
        ...
        boolean allProductsMatch = cartProducts.stream()
                .allMatch(cp -> selectedProducts.stream().anyMatch(sp -> sp.getProductId().equals(cp.getProduct().getId())));
        ...
        UserAddress userAddress = userAddressRepository.findById(userAddressId).orElseThrow(() -> new RuntimeException("User address not found"));
    ...
    private void moveUnselectedProductsToNewCart(List<CartProductRequest> selectedProducts, List<CartProduct> cartProducts, Cart oldCart, Cart newCart) {
        ...
        for (CartProduct cartProduct : cartProducts) {
            ...
            boolean isSelected = selectedProducts.stream()
                    .anyMatch(sp -> sp.getProductId().equals(cartProduct.getProduct().getId()));
    ...
    private void createNotification(Order order, User user, String status) {
        ...
        notification.setContent("Order " + order.getId() + " has been " + status + ". See you again in your next orders."); // Nội dung thông báo
