...
public class CommentController {
    ...
    ApiResponse<PageResponse<CommentResponse>> getAllComments(
        ...
        return ApiResponse.<PageResponse<CommentResponse>>builder().result(commentService.getAllComments(page, size, sortBy, sortDirection)).build();
    }

    @PostMapping("/create")
    ApiResponse<CommentResponse> createCommentAndRating(@RequestParam String userId, @RequestParam String productId, @RequestBody @Valid CommentRequest request) {
        return ApiResponse.<CommentResponse>builder().result(commentService.addCommentAndRating(userId, productId, request)).build();
    }

    @GetMapping("/check")
    ApiResponse<Boolean> checkUserPurchasedProduct(@RequestParam String productId, @RequestParam String userId) {
        return ApiResponse.<Boolean>builder().result(commentService.checkUserPurchasedProduct(userId, productId)).build();
    }

    @PutMapping("/update/{id}")
    ApiResponse<CommentResponse> updateCommentAndRating(@PathVariable Long id, @RequestBody @Valid CommentRequest request) {
        return ApiResponse.<CommentResponse>builder().result(commentService.editCommentAndRating(id, request)).build();
    }
    ...
    public ApiResponse<CommentResponse> addAdminResponse(
        ...
        return ApiResponse.<CommentResponse>builder().result(commentService.addAdminResponse(id, request)).build();
    }
}
