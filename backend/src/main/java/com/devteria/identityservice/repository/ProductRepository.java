...
//    boolean existsByIsbn13(String isbn13);
    ...
    // It checks if a product with the given title, ISBN10, or ISBN13 exists excluding the current product being updated.
    ...
    Page<Product> filterProducts(Long categoryId, Long publisherId, List<String> formality, BigDecimal minPrice, BigDecimal maxPrice, String title, Pageable pageable);

    @Query("SELECT p FROM Product p WHERE " +
            "(:keyword IS NULL OR CAST(p.id AS string) = :keyword) OR " +
