...
import com.ntn.ecommerce.mapper.PublisherMapper;
...
public class PublisherService {
    ...
    public PublisherResponse updatePublisher(Long id, PublisherRequest request) {
        ...
        if (existingPublisherOpt.isPresent()) {
            ...
            if (!existingPublisher.getName().equals(request.getName())
                    && publisherRepository.existsByName(request.getName())) {
                throw new AppException(ErrorCode.PUBLISHER_NAME_EXISTED);
            }
            if (!existingPublisher.getCode().equals(request.getCode())
                    && publisherRepository.existsByCode(request.getCode())) {
                throw new AppException(ErrorCode.PUBLISHER_CODE_EXISTED);
            }
    ...
    public PublisherResponse getPublisherById(Long id) {
        Publisher publisher =
                publisherRepository.findById(id).orElseThrow(() -> new AppException(ErrorCode.PUBLISHER_NOT_FOUND));
        return publisherMapper.toPublisherResponse(publisher);
    }
    ...
    private Pageable createPageable(int page, int size, String sortBy, String sortDirection) {
        ...
        Sort.Direction direction =
                Sort.Direction.fromOptionalString(sortDirection).orElse(Sort.Direction.ASC);
